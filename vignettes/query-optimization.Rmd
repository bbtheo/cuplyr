---
title: "Query Optimization in cuplyr"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Query Optimization in cuplyr}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = identical(Sys.getenv("CUPLYR_VIGNETTE_EVAL"), "true") && cuplyr::has_gpu()
)
```

## Introduction

When you use cuplyr in lazy mode, operations aren't executed immediately.
Instead, they build an Abstract Syntax Tree (AST) that represents your query.
Before execution, the AST passes through an optimizer that rewrites the query
...
When you write a dplyr pipeline in lazy mode, each operation creates a node
in the AST. The tree grows from bottom (source data) to top (final operation).
...
|------|----------|-------------|
| `tbl_gpu()` | `source` | Leaf node with schema metadata |
| `filter()` | `filter` | Row filtering predicates |
| `mutate()` | `mutate` | Column transformations |
| `select()` | `select` | Column projection |
| `arrange()` | `arrange` | Sort specification |
| `group_by()` | `group_by` | Grouping metadata |
| `summarise()` | `summarise` | Aggregation operations |
| `head()` | `head` | Row limit |
| `collapse()` | `barrier` | Optimization fence |

### How the AST Grows

Each operation wraps the previous one:

```{r ast-growth, eval=FALSE}
# This pipeline:
...
The source node is at the bottom. Operations stack on top. When executed,
the tree is traversed from bottom to top.

## The Optimization Pipeline

Before execution, the AST passes through six optimization passes in order:
...
- Filter passes run last since earlier passes may create new opportunities

## Pass 1: Projection Pruning

This pass pushes column selections down the tree to reduce data width early.

### Before Optimization

```{r proj-before, eval=FALSE}
# User writes:
tbl_gpu(mtcars, lazy = TRUE) |>
  mutate(power_ratio = hp / wt) |>
...
The optimizer inserts a `select` node below the `mutate` to fetch only the
columns needed (`mpg`, `hp`, `wt`):

```{r proj-after, eval=FALSE}
...
On wide tables, projection pruning dramatically reduces memory bandwidth.
Instead of copying all columns through every operation, only needed columns
flow through the pipeline.

## Pass 2: Mutate Fusion

Consecutive `mutate()` calls are combined when safe.

### Before Optimization

```{r mutate-fusion-before, eval=FALSE}
...
```{r mutate-fusion-after, eval=FALSE}
...
- Too many intermediate columns are reused (>4 intermediates or >3 uses each)

### Dependency Handling

When fusing creates dependencies, expressions are topologically sorted:

```{r topo-sort, eval=FALSE}
# Input expressions:
#   c = a + b
#   a = hp + 10
#   b = wt * 2
...
After projection pushdown and mutate fusion, some computed columns may no
longer be used. This pass removes them.

### Example

```{r dead-column, eval=FALSE}
# User writes:
tbl_gpu(mtcars, lazy = TRUE) |>
  mutate(
    a = hp + 10,
...
This pass walks backward through the AST, tracking which columns are actually
...
`filter()` operations closer to the data source, reducing the number of rows
...
A filter can be pushed below a `mutate` when the filter's predicate columns
are **not** produced by that mutate.
...
Filters can also push below `select()` if the select keeps the predicate columns:

```{r pushdown-select, eval=FALSE}
# Original:
...
2. When encountering a `filter` node:
   ...
   - If mutate's output columns don't overlap with predicate columns, swap them
3. Repeat until no more pushdowns are possible

```{r pushdown-algorithm, eval=FALSE}
...
Consider a 100-million row dataset where `filter(x > threshold)` keeps 10% of
rows:
...
- **With pushdown**: filter first (minimal allocation), then mutate allocates 10M-row output

The memory savings compound through the pipeline. With multiple mutates and
filters, pushdown can reduce peak memory by 10x or more.

## Pass 5: Filter Reordering

When multiple filter predicates exist, they're reordered by estimated cost.
...
By running cheaper filters first, more rows are eliminated before expensive
comparisons.
...
Some operations act as **barriers** that prevent optimization across them:

- `arrange()` - Sort order must be respected
- `head()` - Row limit must apply at specific point
- `summarise()` - Aggregation changes row count
- `collapse()` - Explicit user barrier

### Why Barriers Exist

Consider:

```{r barrier-example, eval=FALSE}
tbl_gpu(data, lazy = TRUE) |>
...
```{r tip-filter-source, eval=FALSE}
...
For most workloads, simply use lazy mode and write natural dplyr code. The
...
- `vignette("getting-started")` - Basic cuplyr usage
